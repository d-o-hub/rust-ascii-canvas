## 2026-07-30 - [Keyboard-Friendly Form Navigation & Dynamic Accessible Labeling]
**Learning:** In highly interactive editor workflows, keyboard navigation parity is essential. Users expecting to change dimensions inside inputs will instinctively press 'Enter' rather than manually locating and clicking 'Apply' buttons. Capturing 'Enter' events to execute submission handlers and programmatically focusing the canvas back streamlines navigation. Additionally, dynamically created input fields (such as layers list entries) must have dynamic `aria-label` tags explicitly set upon creation, as standard document-level static labels cannot map to dynamic content.
**Action:** When designing input forms, always attach 'Enter' keydown listeners to execute submits and restore canvas/main-focus. When rendering dynamic DOM list entries with input fields, assign explicit dynamic `aria-label` tags referencing their specific index or name.
